...
use crate::webhook_config::parse_webhook_output;
...
pub(super) fn parse_index_config(
    raw: RawDatalensIndexConfig,
) -> Result<DatalensIndexConfig, IndexerError> {
    ...
    let checkpoint = match raw.checkpoint {
        Some(raw) => {
            parse_checkpoint(raw, &mut errors).unwrap_or_else(|| crate::CheckpointPolicy::File {
                path: PathBuf::new(),
            })
        }
        None => {
