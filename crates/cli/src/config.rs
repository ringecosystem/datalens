...
pub fn validate_config(config: &DatalensConfig) -> Result<(), DatalensError> {
    ...
    match config.storage.backend.as_str() {
        ...
        "s3" => {
            ...
            if s3.runtime_worker_threads == 0 {
                return Err(DatalensError::new(
                    DatalensErrorKind::InvalidInput,
                    "storage.s3.runtime_worker_threads must be greater than zero",
                ));
            }
            if s3.max_concurrent_operations == 0 {
                return Err(DatalensError::new(
                    DatalensErrorKind::InvalidInput,
    ...
    if config.storage.compaction.enabled {
            ...
            <= config.storage.compaction.max_tick_duration_ms
        {
            return Err(DatalensError::new(
                DatalensErrorKind::InvalidInput,
                "storage.compaction.leader_lock_ttl_ms must be greater than storage.compaction.max_tick_duration_ms when compaction is enabled",
            ));
        }
    }
...
fn validate_warmup_config(config: &DatalensConfig) -> Result<(), DatalensError> {
    ...
    if config.warmup.max_durable_intents_per_loop == 0 {
        return Err(DatalensError::new(
            DatalensErrorKind::InvalidInput,
