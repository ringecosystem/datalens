...
fn test_handle_message_accepted_page_writes_business_rows_and_checkpoint() {
    ...
    let page = message_page("cursor-2", MESSAGE_ACCEPTED_DATA);
    ...
    let row = db
        .transaction(|tx| {
            Ok(tx.query_row(
                "SELECT message_hash, source_chain_id, target_chain_id, sender, receiver, raw_event_json
                 FROM ormp_messages",
                [],
                |row| {
                    Ok((
        ...
        .expect("message row");
    assert_eq!(row.0, MESSAGE_ACCEPTED_MSG_HASH);
    assert_eq!(row.1, 1);
    assert_eq!(row.2, 46);
    assert_eq!(row.3, "0x13b2211a7ca45db2808f6db05557ce5347e3634e");
    assert_eq!(row.4, "0x2cd1867fb8016f93710b6386f7f9f1d540a60812");
    let raw_event: serde_json::Value = serde_json::from_str(&row.5).expect("raw event json");
    assert_eq!(raw_event["decodeStatus"], "decoded");
    assert_eq!(raw_event["payload"]["data"], MESSAGE_ACCEPTED_DATA);
}
...
fn test_handle_message_accepted_page_is_idempotent_for_duplicate_events() {
    let db = migrated_db();
    let handler = MessageAcceptedHandler::new("ormp-message-consumer");
    handle_message_accepted_page(
        &db,
        &handler,
        message_page("cursor-2", MESSAGE_ACCEPTED_DATA),
    )
    .expect("first page");

    let summary = handle_message_accepted_page(
        &db,
        &handler,
        message_page("cursor-2", MESSAGE_ACCEPTED_DATA),
    )
...
fn test_checkpoint_does_not_advance_when_business_write_fails() {
    ...
    let err = handle_message_accepted_page(
        &db,
        &handler,
        message_page("cursor-2", MESSAGE_ACCEPTED_DATA),
    )
...
#[test]
fn test_handle_message_accepted_page_skips_failed_decode_without_business_write() {
    let db = migrated_db();
    let handler = MessageAcceptedHandler::new("ormp-message-consumer");
    let page = message_page("cursor-2", "0x1234");
    ...
    assert_eq!(db.message_count().expect("message count"), 0);
    assert_eq!(
        db.checkpoint("ormp-message-consumer").expect("checkpoint"),
        Some("11".to_owned())
    );
...
fn message_page(
    ...
    let server = MockGraphqlServer::new(vec![graphql_page(cursor, data, false)]);
    let client = DatalensOrmpClient::new(sdk_client(&server));
...
fn graphql_page(cursor: &str, data: &str, _has_next_page: bool) -> serde_json::Value {
    json!({
        "data": {
            "query": {
