...
test("runExample uses native queries for every configured live smoke workload", async () => {
  const calls: string[] = [];
  const queries = buildExampleQueries({
    endpoint: "http://127.0.0.1:3000",
    application: "token-sdk-typescript",
    ethereum: { fromBlock: 19000000, toBlock: 19000010 },
  ...
  assert.deepEqual(calls, ["evm.logs", "solana.transactions", "tron.events"]);
...
test("buildExampleQueries uses official token targets and bounded ranges", () => {
  ...
  assert.deepEqual(queries.ethereum, {
    ...
    range: { kind: "block", start: 19000000, end: 19000010 },
...
test("buildRuntimeConfig reads endpoint token application and range overrides", () => {
  ...
  assert.deepEqual(config.ethereum, { fromBlock: 19100000, toBlock: 19100001 });
...
test("formatters print normalized event and cache summaries", () => {
  ...
  const nativeResponse: NativeQueryResponse = {
    ...
    rows: {
      dataset_key: { family: "solana", name: "transactions" },
      rows: {
        dataset: "adapter_json",
        rows: {
          dataset_key: { family: "solana", name: "transactions" },
